...
public abstract class BaseGame extends SurfaceView implements SurfaceHolder.Callback, Runnable{
    ...
    User user;
    MyMediaPlayer media_music;
    MySoundPool sound_music;
    ...
    public BaseGame(Context context, Handler handler){
        ...
        media_music = new MyMediaPlayer(context);
        sound_music = new MySoundPool(context);
        if(OfflineActivity.isMusicOn || MainActivity.isMusicOn){
            media_music.playBgm();
        }
    }
            ...
            for (AbstractEnemyAircraft enemyAircraft : enemyAircrafts) {
                ...
                if (enemyAircraft.crash(bullet)) {
                    ...
                    if (enemyAircraft.notValid()) {
                        ...
                        if(enemyAircraft instanceof BossEnemy && (OfflineActivity.isMusicOn || MainActivity.isMusicOn)){
                            media_music.playBgm();
                        }
                    }
                }
            ...
            if (heroAircraft.crash(flyingSupply) || flyingSupply.crash(heroAircraft)) {
                if(OfflineActivity.isMusicOn || MainActivity.isMusicOn){
                    if(flyingSupply instanceof BombSupply){
                        sound_music.explosionSP();
                    } else if (flyingSupply instanceof HpSupply){
                        sound_music.supplySP();
                    } else if (flyingSupply instanceof FireSupply){
                        sound_music.supplySP();
                    }
                }
...
//            gameOverFlag = true;
//            mbLoop = false;
//            Message message = GameActivity.mainThreadHandler.obtainMessage(1);
//            GameActivity.mainThreadHandler.sendMessage(message);
...
//        }
        ...
        if(heroAircraft.getHp() <= 0){
            ...
            // 获取当前时间
            Date date = new Date();
            SimpleDateFormat formatter = new SimpleDateFormat("EEE MMM dd HH:mm:ss 'GMT' yyyy", Locale.US);
            user.setWriteTime(formatter.format(date));

            mbLoop = false;

            if(OfflineActivity.isMusicOn || MainActivity.isMusicOn){
                sound_music.gameOverBgm();
                media_music.stopBgm();
                media_music.stopBossBgm();
            }
        }
    ...
    private void paintScoreAndLife() {
        ...
        canvas.drawText("LIFE:" + heroAircraft.getHp(), x, y + 50, mPaint);
        if(MainActivity.singleOrOnline == false) {
            canvas.drawText("Enemy Score:" + GameActivity.enemyScore, x+300, y, mPaint);
        }
    }
    ...
    public void run() {
        /*TODO*/
        while(mbLoop || MainActivity.onlineStart){
            Log.d("mbLoop",String.valueOf(mbLoop));
            Log.d("onlineStart",String.valueOf(MainActivity.onlineStart));
            draw();
            if(mbLoop) {
                action();
            }
